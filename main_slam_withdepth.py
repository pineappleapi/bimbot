...
from pyslam.io.ground_truth import GroundTruthType, groundtruth_factory #NEW: included GroundTruthType
...
if __name__ == "__main__":
    ...
    """NEW: Enable depth estimator, uncomment for original
                ...
                if img is not None:
                    ...
                    if depth is None and depth_estimator:
                        ...
                        if Parameters.kDepthEstimatorRemoveShadowPointsInFrontEnd:
                            ...
                            printer.green("Filtered shadow points from depth prediction")
                        else:
                            depth = depth_prediction

                        print("Depth estimation time: %.3f s" % (time.time() - time_start))
                        
                        # NEW: Uncomment to visualize depth prediction
    ...
    """ NEW: Remove comment if you want by exit the ff is saved:   
    try:
        est_poses, timestamps, ids = slam.get_final_trajectory()
        ...
        #NEW: remove evaluate_semantic mapping because it is not applicable
