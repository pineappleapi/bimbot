...
class Parameters:
    ...
    kUseMotionBlurDection = False  #NEW: Use motion blur detection to find inliers and estimate the inter-frame transformation (assuming frames are very close in space). Only between consecutive frames.
    ...
    #NEW: DEPTH ESTIMATION enables
    kVolumetrFicIntegrationUseDepthEstimator = (
        False  # Use depth estimator for volumetric integration in the back-end. Switch to True or False as needed.
    )
    ...
    kVolumetricIntegrationDepthEstimatorType = "DEPTH_ANYTHING_V2"  # "DEPTH_PRO","DEPTH_ANYTHING_V2, "DEPTH_SGBM", "DEPTH_RAFT_STEREO", "DEPTH_CRESTEREO_PYTORCH"  (see depth_estimator_factory.py)
