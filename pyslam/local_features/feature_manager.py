...
class KeyPointFilterTypes(Enum):
    ...
    OCTREE_NMS = (
        4  # Distribute keypoints by using a octree (as a matter of fact, a quadtree): from ORBSLAM2
    )
...
class FeatureManager:
    def __init__(
        ...
        if self.descriptor_type in [
            ...
            Printer.orange(
                "FeatureManager: Forcing scale factor=2 for detector", self.descriptor_type.name
            )
        ...
        if (
            ...
            self.use_pyramid_adaptor = self.num_levels > 1  # override a pyramid adaptor?
            self.pyramid_do_parallel = (
                False  # parallel computations generate segmentation fault (is MSER thread-safe?)
            )
            # self.pyramid_type = PyramidType.GAUSS_PYRAMID
            # self.first_level = 0
            self.need_nms = self.num_levels > 1
        ...
        if self.is_detector_equal_to_descriptor:
            Printer.green(
                "FeatureManager: Using same detector and descriptor object: ",
            ...
            if (
                ...
                if self.detector_type != FeatureDetectorTypes.XFEAT:
                    raise ValueError(
                        "You cannot use XFEAT descriptor without XFEAT detector!\nPlease, select XFEATas both descriptor and detector!"
                    )
                ...
                if self.detector_type != FeatureDetectorTypes.ALIKED:
                    raise ValueError(
                        "You cannot use ALIKED internal descriptor without ALIKED detector!\nPlease, select ALIKED as both descriptor and detector!"
                    )
                self._feature_descriptor = self._feature_detector  # reuse detector object
                #
                #
                ...
                if self.detector_type != FeatureDetectorTypes.LIGHTGLUESIFT:
                    raise ValueError(
                        "You cannot use LIGHTGLUESIFT internal descriptor without LIGHTGLUESIFT detector!\nPlease, select LIGHTGLUESIFT as both descriptor and detector!"
                    )
                self._feature_descriptor = self._feature_detector  # reuse detector object
                #
                #
    ...
    def set_num_features(self, num_features):
        ...
        if self._feature_detector is not None:
            try:
                ...
                Printer.orange(
                    "WARNING: could not set nFeatures in detector: ", self.detector_type.name
                )
        if self._feature_descriptor is not None:
            try:
                ...
                Printer.orange(
                    "WARNING: could not set nFeatures in descriptor: ", self.detector_type.name
                )
    ...
    def compute(self, frame, kps, filter=True):
        if self._feature_descriptor is None:
            Printer.orange(
                'WARNING: descriptor is null according to your settings (are you using a "pure" matcher?), returning None'
            )
            return kps, None
        if (
            not self.need_color_image and frame.ndim > 2
    ...
    def detectAndCompute(self, frame, mask=None, filter=True):
        if self._feature_detector is None and self._feature_descriptor is None:
            Printer.orange(
                'WARNING: detector and/or descriptor are null according to your settings (are you using a "pure" matcher?), returning None'
            )
            return None, None
        if (
            not self.need_color_image and frame.ndim > 2
        ...
        if self.use_pyramid_adaptor:
            ...
            #
                ...
                filter = (
                    False  # disable keypoint filtering since we already applied it for detection
                )
        elif self.use_bock_adaptor:
