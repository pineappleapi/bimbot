...
def unpackSiftOctaveIntra(kpt):
    ...
    return (
        octave + 1
    ) * 3 + layer  # return a virtual intra-level with a virtual scale factor =  2^(1/3) where 3 is the number of layers per octave
...
def descriptor_sigma_mad(des1, des2, descriptor_distances=l2_distances):
    ...
    dists_median = np.median(
        dists
    )  # MAD, approximating dists_median=0  in the computation MAD=median(dists - dists_median)
...
def sat_num_features(kps, des=None, num_features=Parameters.kNumFeatures):
    if len(kps) > num_features:
        ...
        if des is None:
            ...
            # print('sat with des')
            order = np.argsort([kp.response for kp in kps])[::-1][
                :num_features
            ]  # [::-1] is for reverse order
...
def kdt_nms(
    kps, des=None, num_features=Parameters.kNumFeatures, r=Parameters.kKdtNmsRadius, k_max=9
):
    ...
    while not complete:
        ...
        for i, kp in enumerate(kps):
            ...
            if not covered_vec[row][col]:  # if the cell is not covered
                ...
                row_min = int(
                    (row - math.floor(width / c)) if ((row - math.floor(width / c)) >= 0) else 0
                )
                row_max = int(
                ...
                col_min = int(
                    (col - math.floor(width / c)) if ((col - math.floor(width / c)) >= 0) else 0
                )
                col_max = int(
...
def compute_ZNCC_between_matched_keypoints(img1, img2, kps1, kps2, window_size=5):
    def extract_windows(img, kps, window_size):
        ...
        for kp in kps:
            ...
            if (
                x - half_window >= 0
                and x + half_window < img.shape[1]
                and y - half_window >= 0
                and y + half_window < img.shape[0]
            ):
                window = img[
...
def stereo_match_subpixel_correlation(
    ...
    num_keypoints = kps_left.shape[0]
    us_right = np.full(num_keypoints, -1.0, dtype=np.float32)
    disparities = np.full(num_keypoints, -1.0, dtype=np.float32)
    dist_idx_dist = np.empty(num_keypoints, dtype=np.float32)
    dist_idx_idx = np.empty(num_keypoints, dtype=np.int32)
    dist_idx_count = 0

    img_h, img_w = image_left.shape

    for i in range(num_keypoints):
        u_r0 = int(round(kps_right[i, 0]))
        u_l = int(round(kps_left[i, 0]))
        v_l = int(round(kps_left[i, 1]))

        if not (w <= v_l < img_h - w and w <= u_l < img_w - w):
            continue

        left_patch = image_left[v_l - w : v_l + w + 1, u_l - w : u_l + w + 1].astype(
            processing_type
        )
        left_patch = left_patch - left_patch[w, w]

        best_dist = 1e10
        best_inc_r = 0
        dists = np.full(2 * l + 1, 1e10, dtype=np.float32)

        for inc_r in range(-l, l + 1):
            start_u_r = u_r0 + inc_r - w
            end_u_r = u_r0 + inc_r + w + 1

            if start_u_r < 0 or end_u_r > img_w:
                continue

            right_patch = image_right[v_l - w : v_l + w + 1, start_u_r:end_u_r].astype(
                processing_type
            )
            right_patch = right_patch - right_patch[w, w]
        ...
        if best_inc_r == -l or best_inc_r == l:
            continue

        idx = l + best_inc_r
        if idx - 1 < 0 or idx + 1 >= dists.shape[0]:
            continue
        ...
        disparity = kps_left[i, 0] - best_u_r

        if min_disparity <= disparity < max_disparity:
            if disparity <= 0:
                disparity = 0.01
                best_u_r = kps_left[i, 0] - 0.01

            disparities[i] = disparity
            us_right[i] = best_u_r
            dist_idx_dist[dist_idx_count] = best_dist
            dist_idx_idx[dist_idx_count] = i
            dist_idx_count += 1

    if dist_idx_count > 0:
        distances = dist_idx_dist[:dist_idx_count]
        indices = dist_idx_idx[:dist_idx_count]
        median_dist = np.median(distances)
        threshold_dist = 1.5 * 1.4826 * median_dist

        valid_mask = distances < threshold_dist
        valid_idxs = indices[valid_mask]
        invalid_idxs = indices[~valid_mask]

        for idx in invalid_idxs:
            us_right[idx] = -1
            disparities[idx] = -1
    else:
        valid_idxs = np.empty(0, dtype=np.int32)
...
def extract_patches_array_cpp(
    ...
    if kPySlamUtilsAvailable:
        kps_tuples = [(kp.pt[0], kp.pt[1], kp.size, kp.angle, kp.response, kp.octave) for kp in kps]
        return pyslam_utils.extract_patches(
            image=img,
            kps=kps_tuples,
            patch_size=patch_size,
            use_orientation=True,
            scale_factor=mag_factor,
            warp_flags=warp_flags,
        )
    else:
        print("using python version extract_patches_array()")
        return extract_patches_array(
            img=img, kps=kps, patch_size=patch_size, mag_factor=mag_factor, warp_flags=warp_flags
        )
        ...
        for y in range(self.num_cells_y):
            for x in range(self.num_cells_x):
                if self.point_map[y, x] >= num_min_points:
                    img[
                        y * self.cell_y : (y + 1) * self.cell_y,
                        x * self.cell_x : (x + 1) * self.cell_x,
                    ] = 255
        return img
