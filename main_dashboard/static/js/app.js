...
form.addEventListener("submit", async (e) => {
    ...
    // Select the button using its class since we didn't give it an ID
    const uploadBtn = document.querySelector(".upload-btn"); 
    const originalText = "Upload the recorded exploration video";
    ...
    // 1. Change Button to "Uploading..." state
    ...
    try {
        ...
        if (response.ok) {
            ...
            uploadBtn.innerText = "❌ Upload failed. Please try again.";
            uploadBtn.classList.add("error");
            
            // Reset button quickly so they can try again
            setTimeout(() => {
                uploadBtn.innerText = originalText;
                uploadBtn.classList.remove("error");
                uploadBtn.disabled = false;
        ...
        // ❌ ERROR STATE (Network/Code Error)
        uploadBtn.innerText = "❌ Error uploading file: " + err;
        uploadBtn.classList.add("error");
